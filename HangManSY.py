...
for x in ranWord:
    ...
    l.append("_")
while tries!=0:
    print("\nWord: "+fill+"\t\tTries Left: "+str(tries))
    while True:
        guess=input("\nGuess a letter: ").lower()
        used.append(guess)
        if used.count(guess)>1:
            for x in "\nTry something which you haven't guessed already...\n":
                print(x,end="")
                sleep(0.04)
            break
        if guess.isalpha() and len(guess)==1:
            ...
            if guess in ranWord:
                fill=""
                for x,y in zip(ranWord,range(len(ranWord))):
                    if guess==x:
                        l[y]=guess
                for x in l:
                    fill+=x+" "
                if ranWord==l:
                    msg="\nOriginal Word: "+fill+'\t\tYOU WON!'
                    for x in msg:
                        print(x,end="")
                        sleep(0.04)
                    tries=0
                ...
                tries+=-1
            if tries==0 and won==False:
                fill=""
                for x in ranWord:
                    fill+=x+" "
                msg="\nOriginal Word: "+fill+'\t\tYOU LOST!'
                for x in msg:
                    print(x,end="")
                    sleep(0.04)
            break
        else:
            for x in "\nInvalid Guess! Only an ALPHABET allowed at a time!\n":
                print(x,end="")
                sleep(0.04)
            break
